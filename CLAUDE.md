...
## Architecture
- `src/channel.js` - MCP server with swarm_send/receive/status/register tools + Redis pub/sub
...
- Per-workspace channels: `swarmcode:ws:<workspace-id>`
- Broadcast channel: `swarmcode:ws:broadcast`
...
## Running
```bash
npx swarmcode init <workspace-id> --redis redis://host:port
```

## Environment
- `SWARMCODE_REDIS_URL` - Redis URL (default: redis://localhost:6379)
- `SWARMCODE_WORKSPACE_ID` - Workspace identifier
